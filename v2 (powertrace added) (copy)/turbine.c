...
recv_uc(struct unicast_conn *c, const rimeaddr_t *from)
{
    ...
    if ((from->u8[0]==blade1 || from->u8[0]==blade2 || from->u8[0]==blade3 || from->u8[0]==blade4) && flag!=1){
...
PROCESS_THREAD(temperature_process, ev, data)
{
  ...
  while(1) {
    ...
    //Sending blade4 into to central node
    if(node_id == blade4) {
        char buffer[6];
	itoa(bladeFour[bld4++],buffer,10);
        const void *message= buffer;
        packetbuf_copyfrom(message, 3);
        unicast_send(&uc, &addr);
    }
